...
class StockDAO extends Connection
{
    ...
    public function FilterbyDescriptionProductsAndProductCode(StockModel $stockModel, string $desc_produto): array
    {
        $statement = $this->pdo
            ->prepare("SELECT desc_produto , 
                ...
                , qtd_entrada
                , media FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo  AND cod_produto = :cod_produto;");
    ...
    public function FilterbyDescriptionProductsAndProductCodeDesc_Material(StockModel $stockModel, string $desc_produto): array
    {
        $statementDescMaterial = $this->pdo
            ->prepare("SELECT 
                ...
                count( CASE WHEN (desc_material = 'FOLHETERIA') THEN 1 END )AS total_folheterias FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo AND cod_produto = :cod_produto;");
    ...
    public function FilterbyDescriptionProduto(StockModel $stockModel, string $desc_produto): array
    {
        $statement = $this->pdo
            ->prepare("SELECT desc_produto , 
                ...
                , to_char(dt_entrada, 'DD/MM/YYYY') AS dt_entrada
                , qtd_entrada
                , media FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo AND desc_produto ILIKE '%" . $desc_produto . "%';");
    ...
    public function FilterbyDescriptionProdutoDesc_Material(StockModel $stockModel, string $desc_produto): array
    {
        $statementDescMaterial = $this->pdo
            ->prepare("SELECT 
                count( CASE  WHEN (desc_material = 'PLÁSTICO') THEN 1 END )AS total_plásticos 
                , count ( CASE  WHEN (desc_material = 'ENVELOPE') THEN 1 END )AS total_envelopes , 
                count( CASE  WHEN (desc_material = 'FOLHETERIA') THEN 1 END )AS total_folheterias FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo AND desc_produto ILIKE '%" . $desc_produto . "%';");
    ...
    public function FilterbyActive(StockModel $stockModel): array
    {
        $statement = $this->pdo
            ->prepare("SELECT desc_produto , 
                ...
                , to_char(dt_entrada, 'DD/MM/YYYY') AS dt_entrada
                , qtd_entrada
                , media FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo ;");
    ...
    public function FilterbyActiveDesc_Material(StockModel $stockModel): array
    {
        $statementDescMaterial = $this->pdo
            ->prepare("SELECT 
                count( CASE  WHEN (desc_material = 'PLÁSTICO') THEN 1 END )AS total_plásticos 
                , count ( CASE  WHEN (desc_material = 'ENVELOPE') THEN 1 END )AS total_envelopes , 
                count( CASE  WHEN (desc_material = 'FOLHETERIA') THEN 1 END )AS total_folheterias FROM  view_redeuze_relatorio_estoque WHERE ativo=:ativo ;");
    ...
    public function FilterAllMaterials(): array
    {
        $statement = $this->pdo
            ->prepare("SELECT 
            count( CASE  WHEN (desc_material = 'PLÁSTICO') THEN 1 END )AS total_plásticos,
            count ( CASE WHEN (desc_material = 'ENVELOPE') THEN 1 END )AS total_envelopes, 
            count( CASE WHEN (desc_material = 'FOLHETERIA') THEN 1 END )AS total_folheterias 
            FROM  view_redeuze_relatorio_estoque;");
    ...
    public function FilterAllData(): array
    {
        $statement = $this->pdo
            ->prepare("SELECT desc_produto, saldo_atual, ativo, cod_produto, desc_material,
            to_char(dt_entrada, 'DD/MM/YYYY') AS dt_entrada, qtd_entrada, media 
            FROM view_redeuze_relatorio_estoque;");
